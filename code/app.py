...
def get_table_info(table_path):
    ...
    info = {
        ...
        "population_fields": [
            f for f in fields if f.endswith("_af") and "gnomad_" in f
        ],
        "n_variants": ht.count(),
    }
    return info
...
def parse_variant_string(variant_str):
    ...
    match = re.match(
        r"(chr\w+)[:-](\d+)[:-]([ACGT]+)[:-]([ACGT]+)", variant_str, re.IGNORECASE
    )
    if match:
...
with st.sidebar:
    ...
    table_path = st.text_input(
        "Hail Table Path", value="variants.ht", help="Path to your Hail table"
    )
        ...
        with st.spinner("Loading table..."):
            try:
                ...
                with st.expander("📊 Table Features", expanded=True):
                    ...
                    st.write(
                        f"**gnomAD:** {'✅' if table_info['has_gnomad'] else '❌'}"
                    )
                    st.write(
                        f"**TOPMed:** {'✅' if table_info['has_topmed'] else '❌'}"
                    )
                    st.write(
                        f"**ClinVar:** {'✅' if table_info['has_clinvar'] else '❌'}"
                    )
                    st.write(
                        f"**Transcripts:** {'✅' if table_info['has_transcripts'] else '❌'}"
                    )
                    st.write(
                        f"**Conservation:** {'✅' if table_info['has_conservation'] else '❌'}"
                    )
...
with col1:
    search_type = st.radio(
        "Search by:", ["Variant", "Region", "Gene", "Browse All"], horizontal=True
    )
...
if search_button or search_type == "Browse All":
    with st.spinner("Searching variants..."):
        try:
            ...
            if search_type == "Variant" and search_query:
                ...
                if rsid:
                    ...
                    if ref and alt:
                        ...
                        filtered_ht = ht.filter(
                            (ht.chromosome == chrom) & (ht.position == pos)
                        )
            ...
            ####
            # arrow_table = filtered_ht.to_arrow()
...
if "current_df" in st.session_state:
    ...
    with metrics[2]:
        if "gnomad_af" in df.columns:
            ...
            st.metric(
                "Mean gnomAD AF", f"{mean_af:.6f}" if not pd.isna(mean_af) else "N/A"
            )
    ...
    with st.expander("🔧 Apply Filters", expanded=False):
        ...
        with filter_cols[0]:
            ...
            if "max_gnomad_af" in df.columns:
                af_min = st.number_input(
                    "Min AF", 0.0, 1.0, 0.0, format="%.6f", key="af_min"
                )
                af_max = st.number_input(
                    "Max AF", 0.0, 1.0, 1.0, format="%.6f", key="af_max"
                )
        ...
        with filter_cols[1]:
            ...
            if "phylop_score" in df.columns:
                min_phylop = st.number_input(
                    "Min PhyloP", -20.0, 20.0, -20.0, key="phylop"
                )
        ...
        if freq_category:
            ...
            for cat in freq_category:
                if cat == "Ultra-rare (<0.01%)":
                    ...
                    freq_mask |= (df["max_gnomad_af"] >= 0.0001) & (
                        df["max_gnomad_af"] < 0.001
                    )
                elif cat == "Low freq (0.1-1%)":
                    freq_mask |= (df["max_gnomad_af"] >= 0.001) & (
                        df["max_gnomad_af"] < 0.01
                    )
                elif cat == "Common (1-5%)":
                    freq_mask |= (df["max_gnomad_af"] >= 0.01) & (
                        df["max_gnomad_af"] < 0.05
                    )
        ...
        if clinvar_filter:
            ...
            for sig in clinvar_filter:
                clinvar_mask |= df["clinvar_significance"].str.contains(
                    sig, case=False, na=False
                )
            mask &= clinvar_mask

        if consequence_filter:
            mask &= df["all_consequences"].str.contains(
                consequence_filter, case=False, na=False
            )
        ...
        if len(df_filtered) < len(df):
            st.info(
                f"📉 After filtering: **{len(df_filtered):,}** variants (from {len(df):,})"
            )
            df = df_filtered
    ...
    with tab1:
        if "gnomad_af" in df.columns or "max_gnomad_af" in df.columns:
            ...
            if "gnomad_af" in df.columns and "gnomad_exome_af" in df.columns:
                ...
                comparison_df = comparison_df.fillna(np.nan)
                comparison_df = comparison_df.astype(object).where(
                    pd.notna(comparison_df), None
                )
    ...
    with tab2:
        ...
        if pop_cols:
            # Population means
            pop_names = [
                c.replace("gnomad_", "").replace("_af", "").upper() for c in pop_cols
            ]
            pop_means = [df[col].mean() for col in pop_cols]

            fig = go.Figure(
                data=[go.Bar(x=pop_names, y=pop_means, marker_color="lightblue")]
            )
            fig.update_layout(
    ...
    with tab4:
        ...
        selected_cols = st.multiselect(
            "Select columns to display:", display_cols, default=display_cols[:10]
        )
    ...
    with tab5:
        ...
        if "all_consequences" in df.columns:
            ...
            consequence_counts = (
                consequences[consequences != ""].value_counts().head(20)
            )
